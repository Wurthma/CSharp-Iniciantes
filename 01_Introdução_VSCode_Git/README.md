...
2. O VS Code possui um CLI, não  abordaremos com detalhes, mas para testar a instalação do mesmo, abra um software de linha de comando e digite `code --version` para verificar a versão do mesmo.
...
3. Localize a extensão do C# e faça a instalação.
    ...
    ![Vs Code Extension](../Resources/Imgs/01_VsCode_Btn_Extension.png)

## Git

Git é um software de controle de versão. Pense nele como o repositório onde você "guardará" seus projetos e controlará quem está fazendo alterações nele.
...
Após instalação do git, antes de usa-lo é necessário configurar um nome de usuário e um e-mail usando os comando abaixo:
...
1. Crie no respositório de sua escolha um projeto com o nome "ControleDeVersaoComGit":
...
1. Pelo "Explorer" do VS Code crie um novo arquivo na raiz do nosso projeto "ControleDeVersaoComGit" e nome-o como .gitignore
2. Coloque este conteúdo no arquivo criado.
3. Salve o arquivo e você terá menos arquivos na notificação do "Source Control".


#### Commit

Um **commit** grava as alterações feitas no projeto. Anter de realizar o commit procure sempre revisar tudo que você alterou antes para garantir que você não alterou nada que comprometa o funcionamento da sua aplicação.
...
Após garantir que sua aplicação está funcionando normalmente, siga os passos abaixo pra fazer o nosso primeiro commit:
...
3. Use novamento o comando `git status` e agora os arquivos foram adicionados ao git.
...
Git e github não são a mesma coisa. O github.com é um site onde podemos subir nossos repositórios pra nuvem. Isso pode ser feito pelos comandos do git.
...
1. Crie um novo repositório em Repositories > New
2. Coloque o nome "ControleDeVersaoComGit"
3. Coloque a descrição que achar mais adequada, deixe o repositório como público e deixa o restante das opções desmarcadas.
...
5. Atualize seu repositório no github e você poderá observar as alterações. Entre també no histórico do seu repositório no github e você visualizará todos seus commits. Navegue pelo histórico e tente entende-lo.
...
2. A URL pode ser visualizada no github pelo botão "Code" do repositório que deseja clonar. Acesse o [repositório que iremos clonar](http://https://github.com/Wurthma/HackerHankExercices "repositório que iremos clonar") e tente encontrar a URL. O comando completo ficará assim:
    `git clone https://github.com/Wurthma/HackerHankExercices.git`
...
Existem ferramentas específicas que facilitam a visualização do merge, o próprio VS Code pode ser utilizado para isso. Mas é possível faze-lo com o comando `git merge`.
